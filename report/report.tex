...
Because the emulator should not rely on the environment it is running it to work, the functionality of the project can be split into two parts: the emulator core, that is responsible for simulating the Gameboy, and the emulator's frontend, that allows interfacing with the user, and may be changed to work for different platforms.
...
\begin{figure}[h]
    ...
    \begin{tikzpicture}
    	...
    	\umlHVassemblyconnector[interface=EmulatorOut, with port, first arm]{EmulatorUI}{Gameboy}
    	\umlHVassemblyconnector[interface=EmulatorIn, with port, last arm]{Gameboy}{EmulatorUI}
    \end{tikzpicture}
