...
var CPU = function(gameboy) {
    ...
    this.r = {A:0x01, F: 0xb0, B:0x00, C:0x13, D:0x00, E:0xd8, H:0x01, L:0x4d, pc:0x0100, sp:0xfffe};
    this.IME = true;
...
CPU.prototype.run = function() {
    if (this.usingBootRom) {
        ...
        this.r.pc = 0x0100;
        this.memory.wb(0xff0f, 0xe1);
        this.memory.wb(0xff41, 0x85);
        this.memory.wb(0xff07, 0xf8);
        this.memory.wb(0xff40, 0x91);
        this.memory[0xff04] = 0xab;
    }
    this.frame();
};
                ...
                if(offset < this.counter && this.counter < offset + limit) {
                    ...
                    this.logOut.push(
                        ...
                        Object.entries(loggedMemory)
                            ...
                            .join('') +
                        `a:${this.r.A.toString(16)}/`+
                        `f:${this.r.F.toString(16)}/`+
                        `b:${this.r.B.toString(16)}/`+
                        `c:${this.r.C.toString(16)}/`+
                        `d:${this.r.D.toString(16)}/`+
...
CPU.prototype.endSerialTransfer = function() {
    ...
    this.serialHandler.out(data, this.counter);
...
var ConsoleSerial = {
    current: '',
    timeout: null,
    out: function(data, counter) {
        ConsoleSerial.current += String.fromCharCode(data);
        ConsoleSerial.counter = counter;
        if (data == 10) {
            ConsoleSerial.print();
        } else {
    ...
    print: function() {
        clearTimeout(ConsoleSerial.timeout);
        console.log(`(${ConsoleSerial.counter}) serial: ${ConsoleSerial.current}`);
